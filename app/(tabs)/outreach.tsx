...
import { useCampaignStore, useCampaigns, useEmailStatus, useIsCampaignLoading } from '@/stores/campaignStore'
import { api } from '@/lib/api'
import { CoachCardSkeleton, CampaignsSkeleton } from '@/components/ui/Skeleton'
import { useToast } from '@/components/ui/Toast'
import { haptics } from '@/lib/haptics'
import { useDrawerStore } from '@/stores/drawerStore'
import { analytics } from '@/lib/analytics'
import { OfflineBanner } from '@/components/ui/OfflineBanner'
import { colors, spacing, fontSize, borderRadius, fontFamily, cardStyles, shadows } from '@/constants/theme'
import type { TwitterCoachMatch, TwitterDMStatus, Division, Campaign, CampaignStatus } from '@/types'

type OutreachChannel = 'twitter' | 'email'
...
function divisionColor(division: Division | string | null): string {
  switch (division) {
    case 'D1_FBS_P4': return '#D4A857'
    case 'D1_FBS_G5': return '#22C55E'
    case 'D1_FCS': return '#3B82F6'
...
const STATUS_COLORS: Record<CampaignStatus, { bg: string; text: string }> = {
  DRAFT: { bg: 'rgba(255,255,255,0.1)', text: colors.textMuted },
  SCHEDULED: { bg: 'rgba(59,130,246,0.15)', text: '#3B82F6' },
  SENDING: { bg: 'rgba(245,158,11,0.15)', text: '#F59E0B' },
  COMPLETED: { bg: 'rgba(34,197,94,0.15)', text: '#22C55E' },
  FAILED: { bg: 'rgba(239,68,68,0.15)', text: '#EF4444' },
  CANCELLED: { bg: 'rgba(255,255,255,0.08)', text: colors.textDim },
}
...
export default function OutreachScreen() {
  ...
  // Channel toggle
  const [channel, setChannel] = useState<OutreachChannel>('twitter')
  ...
  const [hasMoreTwitter, setHasMoreTwitter] = useState(true)
  const [isTwitterLoading, setIsTwitterLoading] = useState(true)
  const [isTwitterLoadingMore, setIsTwitterLoadingMore] = useState(false)
  ...
  const isEmailLoading = useIsCampaignLoading()
  const fetchCampaigns = useCampaignStore((s) => s.fetchCampaigns)
  const fetchEmailStatus = useCampaignStore((s) => s.fetchEmailStatus)
  const hasEmailConnection = !!emailStatus?.activeConnection
  ...
  useFocusEffect(
    useCallback(() => {
      loadTwitterFollowers(true)
      fetchCampaigns()
      fetchEmailStatus()
    }, [])
  )

  // Reload Twitter when filters change
  useEffect(() => {
    if (!isTwitterLoading && channel === 'twitter') {
      loadTwitterFollowers(true)
    }
  ...
  // Debounced search
  useEffect(() => {
    if (channel !== 'twitter') return
    if (searchTimer.current) clearTimeout(searchTimer.current)
    searchTimer.current = setTimeout(() => {
      if (!isTwitterLoading) loadTwitterFollowers(true)
    }, 400)
    return () => { if (searchTimer.current) clearTimeout(searchTimer.current) }
  }, [searchQuery])

  const loadTwitterFollowers = async (reset = false) => {
    if (reset) {
      setIsTwitterLoading(true)
      setTwitterOffset(0)
    } else {
      setIsTwitterLoadingMore(true)
    }

    const nextOffset = reset ? 0 : twitterOffset
    ...
    try {
      ...
      setTwitterOffset(nextOffset + data.matches.length)
      setHasMoreTwitter(data.matches.length >= PAGE_SIZE)
    } catch (err) {
      // UI shows empty/error state on failure
    } finally {
      setIsTwitterLoading(false)
      setIsTwitterLoadingMore(false)
    }
  }

  const handleTwitterEndReached = useCallback(() => {
    if (!isTwitterLoadingMore && hasMoreTwitter && !isTwitterLoading) {
      loadTwitterFollowers(false)
    }
  }, [isTwitterLoadingMore, hasMoreTwitter, isTwitterLoading, twitterOffset, selectedDivision, positionFilter, searchQuery])
  ...
  const handleRefresh = async () => {
    if (channel === 'twitter') {
      await loadTwitterFollowers(true)
    } else {
      await Promise.all([fetchCampaigns(), fetchEmailStatus()])
    }
  }

  // --- Render Twitter Coach Card ---
  const renderTwitterCard = useCallback(({ item }: { item: TwitterCoachMatch }) => {
  ...
  const renderCampaignCard = useCallback(({ item }: { item: Campaign }) => {
    const openRate = item.sentCount > 0
      ? Math.round((item.openCount / item.sentCount) * 100)
      : 0

    return (
      <TouchableOpacity
        style={[cardStyles.base, styles.campaignCard]}
      ...
      >
        ...
        <View style={styles.campaignStats}>
          <View style={styles.stat}>
            ...
            <Text style={styles.statText}>{item.totalRecipients}</Text>
          </View>
          <View style={styles.stat}>
            <Ionicons name="send-outline" size={16} color={colors.textMuted} />
            <Text style={styles.statText}>{item.sentCount}</Text>
          </View>
          <View style={styles.stat}>
            <Ionicons name="mail-open-outline" size={16} color={colors.textMuted} />
            <Text style={styles.statText}>{openRate}%</Text>
          </View>
  ...
  const isLoading = channel === 'twitter' ? isTwitterLoading : (isEmailLoading && campaigns.length === 0)
  ...
  return (
    <View style={[styles.container, { paddingTop: insets.top }]}>
      ...
      <View style={styles.header}>
        ...
        {channel === 'twitter' && twitterConnected && (
          <TouchableOpacity
        ...
        {channel === 'email' && hasEmailConnection && (
          <TouchableOpacity onPress={handleCreateCampaign} style={styles.addBtn}>
            <Ionicons name="add" size={24} color={colors.primary} />
          </TouchableOpacity>
        )}
        {((channel === 'twitter' && !twitterConnected) || (channel === 'email' && !hasEmailConnection)) && (
          <View style={{ width: 60 }} />
        )}
      </View>
      ...
      <View style={styles.channelToggle}>
        <TouchableOpacity
          style={[styles.channelBtn, channel === 'twitter' && { backgroundColor: primaryColor }]}
          onPress={() => { setChannel('twitter'); haptics.light() }}
        >
          <Text style={[styles.channelBtnText, channel === 'twitter' && { color: colors.background }]}>
            X / DMs
          </Text>
        </TouchableOpacity>
        <TouchableOpacity
          style={[styles.channelBtn, channel === 'email' && { backgroundColor: primaryColor }]}
          onPress={() => { setChannel('email'); haptics.light() }}
        >
          <Text style={[styles.channelBtnText, channel === 'email' && { color: colors.background }]}>
            Email
          </Text>
        </TouchableOpacity>
      </View>
      ...
      {channel === 'twitter' && (
        <>
          {/* Not connected state */}
          {!isTwitterLoading && !twitterConnected && (
            <View style={styles.connectContainer}>
              ...
              <Button title="Connect Twitter/X" onPress={handleConnectTwitter} fullWidth />
            </View>
          )}
            ...
            <>
              ...
              <View style={styles.filterBar}>
                ...
                <View style={styles.searchRow}>
                  ...
                  />
                  <Text style={styles.matchCountText}>{matches.length} match{matches.length !== 1 ? 'es' : ''}</Text>
                </View>
              </View>
              ...
              <FlatList
                ...
                renderItem={renderTwitterCard}
                contentContainerStyle={[styles.listContent, { paddingBottom: insets.bottom + spacing.xl }]}
                onEndReached={handleTwitterEndReached}
                onEndReachedThreshold={0.4}
                refreshControl={
                  <RefreshControl
                    refreshing={isTwitterLoading}
                    onRefresh={() => loadTwitterFollowers(true)}
                    tintColor={primaryColor}
                  />
                }
                ListFooterComponent={
                  isTwitterLoadingMore ? (
      ...
      {channel === 'email' && (
        <>
            ...
            >
              <View style={styles.emailBannerContent}>
                ...
                <Text style={styles.emailBannerText}>
                  Sending from {emailStatus.activeConnection.email}
                </Text>
              </View>
          ...
          {/* Loading */}
          {isEmailLoading && campaigns.length === 0 && (
            <CampaignsSkeleton />
          )}
          ...
          {!isEmailLoading && campaigns.length === 0 && (
            <View style={styles.emptyState}>
              <Ionicons
                name={hasEmailConnection ? 'mail-outline' : 'mail-unread-outline'}
                size={48}
                color={colors.textMuted}
              />
              <Text style={styles.emptyStateTitle}>
                {hasEmailConnection ? 'No Campaigns Yet' : 'Connect Your Email'}
              </Text>
              <Text style={styles.emptyStateText}>
                {hasEmailConnection
                  ? 'Create your first email campaign to reach out to multiple coaches at once.'
                  : 'Connect your Gmail or Outlook to start sending personalized emails to coaches.'}
              </Text>
              <TouchableOpacity
                style={styles.emptyBtn}
                onPress={hasEmailConnection ? handleCreateCampaign : handleConnectEmail}
                activeOpacity={0.8}
              >
                <Ionicons
                  name={hasEmailConnection ? 'add' : 'link'}
                  size={20}
                  color={colors.background}
                />
                <Text style={styles.emptyBtnText}>
                  {hasEmailConnection ? 'Create Campaign' : 'Connect Email'}
                </Text>
              </TouchableOpacity>
            </View>
          )}
          ...
          {/* FAB */}
          {hasEmailConnection && campaigns.length > 0 && (
            <TouchableOpacity
